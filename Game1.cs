...
namespace Aayan_s_Grade_11_Final_Project
{
    public class Game1 : Game
    {
        ...
        double score = 0;
        int shipLives = 3;
        int alienShootChance = 1300;
        int bossHealth = 100;
        int bossShootChance = 60;
        int endlessWave = 0;
        ...
        void SpawnWave()
        {
            ...
            alienTextures.Clear();
            alienLasers.Clear();
            alienLaserColors.Clear();

            alienDirection = 1;
            endlessWave++;
            alienSpeed++;

            if (alienShootChance > 100)
            {
                alienShootChance -= 100;
            }
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            if (currentScreen == Screen.Single)
            {
                ...
                for (int a = 0; a < aliens.Count; a++)
                {
                    if (rng.Next(alienShootChance) == 1)
                    {
                        ...
                        alienLaserColors.Add(alienColor[a]);
                    }
                }
            ...
            if (currentScreen == Screen.Endless)
            {
                if (keyboard.IsKeyDown(Keys.Left) && ship.X > 0)
                {
                    ship.X -= shipSpeed;
        ...
        protected override void Draw(GameTime gameTime)
        {
            ...
            else if (currentScreen == Screen.Single)
            {
                ...
                {
                    _spriteBatch.Draw(alienLaserTexture, alienLasers[i], alienLaserColors[i]);
                }
            ...
            else if (currentScreen == Screen.Endless)
            {
                _spriteBatch.Draw(backgroundTexture, window, Color.White);
                _spriteBatch.Draw(shipTexture, ship, Color.White);
                _spriteBatch.DrawString(font, "Wave: " + endlessWave, new Vector2(10, 10), Color.White);
                _spriteBatch.DrawString(font, "Score: " + score, new Vector2(338, 10), Color.White);
